...
try:
    ...
    print("Copy config.py from the repo, fill in your local paths, and try again.")
...
def find_image(img_name, frames_dir):
    ...
    # Search all subdirs by filename
...
def parse_all_xmls(xml_dir, frames_dir):
    ...
    print(f"Skipped (no boxes): {skipped_no_box}")
...
def train_model(yaml_path):
    ...
    print(f"Training for {EPOCHS} epochs, this could take a while!\n")
...
if __name__ == "__main__":
    ...
    if len(annotations) == 0:
        ...
        print("Check that FRAMES_DIR contains your JPEG frame subfolders.")
        exit(1)
